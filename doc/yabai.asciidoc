...
The action domain exposes Yabai's native command palette and stable action identifiers suitable for skhd bindings.

Rules, signals, and raw queries are intentionally not included in the palette because they are better suited to configuration files or terminal output.

General Syntax
^^^^^^^^^^^^^^

yabai -m action '<COMMAND>'

COMMAND
^^^^^^^

*--show*::
    Show the native command palette on the display containing the pointer. +
    Type to filter by title, action identifier, category, or syntax. +
    Use the arrow keys to navigate and Return to select an action. +
    Actions requiring an argument transition to a text-input view. +
    Escape returns from argument or confirmation views to the action list, and closes the palette from the action list. +
    Destructive palette actions require confirmation.

*--run* '<ACTION_ID>' ['<ARGUMENT>']::
    Execute an action directly by its stable identifier. +
    Quote arguments containing spaces so they arrive as a single argument. +
    Direct low-level execution returns the underlying IPC command output and error status. +
    Native workflow actions open their interactive view when required. +
    Destructive actions invoked directly do not show the palette confirmation view.

*--list*::
    Output the action catalog as JSON, including identifiers, titles, categories, action kinds, argument requirements, syntax, and destructive flags.

ACTION_ID
^^^^^^^^^

Display actions::
    'display.focus', 'display.space', 'display.label'

Space actions::
    'space.choose', 'space.relabel', 'space.layout-cycle', 'space.focus', 'space.switch', 'space.create', 'space.destroy', 'space.move', 'space.swap', 'space.display', 'space.equalize', 'space.balance', 'space.mirror', 'space.rotate', 'space.padding', 'space.gap', 'space.toggle', 'space.layout', 'space.label'

Native space workflows::
    'space.choose' opens a searchable list of spaces and creates, labels, and focuses a space from unmatched search text. +
    'space.relabel' prompts with the focused space's current label and clears the label when submitted empty. +
    'space.layout-cycle' cycles the focused space through stack, bsp, and float layouts.

Window actions::
    'window.focus', 'window.close', 'window.minimize', 'window.deminimize', 'window.display', 'window.space', 'window.swap', 'window.warp', 'window.stack', 'window.insert', 'window.grid', 'window.move', 'window.resize', 'window.ratio', 'window.toggle', 'window.sub-layer', 'window.opacity', 'window.raise', 'window.lower', 'window.scratchpad', 'window.stack-selector-anchor'

Runtime-setting actions::
    'config.mouse-follows-focus', 'config.focus-follows-mouse', 'config.stack-selector', 'config.stack-selector-anchor', 'config.skip-window-focus-animation', 'config.layout', 'config.split-type', 'config.split-ratio', 'config.top-padding', 'config.bottom-padding', 'config.left-padding', 'config.right-padding', 'config.window-gap', 'config.auto-balance'
...
*stack_selector* ['<BOOL_SEL>']::
    Show a clickable application-icon selector for every window stack. +
    Hovering an inactive window icon displays a window preview. +
    Right-clicking the selector opens a visual anchor grid for setting that stack's anchor or restoring the global default. +
    Applies to both stack layout and stacked leaf-nodes in bsp layout.

*stack_selector_anchor* ['<STACK_SELECTOR_ANCHOR>']::
    Set the default anchor used by stack selectors. +
    Valid anchors are: vertical-top-left, vertical-center-left, vertical-bottom-left, vertical-top-right, vertical-center-right, vertical-bottom-right, horizontal-left-top, horizontal-center-top, horizontal-right-top, horizontal-left-bottom, horizontal-center-bottom, horizontal-right-bottom.

*status_island* ['<BOOL_SEL>']::
    Show a dynamic-island style status pill merged with the notch. +
    The left wing shows the focused workspace name and the right wing shows the focused space layout. +
    Clicking the left wing opens a workspace selector; clicking the right wing opens a layout selector. +
    Enabled by default.

*status_island_workspace_order* ['<index|alphabetical|creation>']::
    Order the remaining workspaces in the workspace selector dropdown. +
    The focused workspace is always shown first. +
    'index' sorts by mission-control index, 'alphabetical' by label, and 'creation' by space creation order.

*space_cleaner* ['<BOOL_SEL>']::
    Automatically destroy empty, non-visible spaces. +
    Runs when a window is destroyed or moved to another space, and when the visible space changes. +
    The visible space and the last remaining user space on each display are never destroyed.
...
*--stack-selector-anchor* ['<STACK_SELECTOR_ANCHOR>|next|prev|default']::
    Set or query the stack-selector anchor for the selected window's stack. +
    The values 'next' and 'prev' cycle through all anchor ports. +
    The value 'default' removes the per-stack override and follows the global 'stack_selector_anchor' setting.
